...
@Component({
  ...
  templateUrl: './attempts-page.component.html',
  styleUrls: ['./attempts-page.component.css']
})
export class AttemptsPageComponent {
  constructor(
  ...
  attemptsViewModel: IPageView<IAttempt> = {
    currentPage: 1,
    totalPages: 1,
    pageSize: 10,
    totalCount: 0,
  ...
  checkForEmptyResult(){
    if (this.attemptsViewModel.rows.length === 0){
  ...
  fetchSolutions(){
    ...
    this.tasksService.getSolutionAttempts(request).subscribe({
      next: (data) => {
        console.log(data);
        this.attemptsViewModel = data;
      ...
      error: (err) => {
        ...
        this.isError = true;
        this.isLoading = false;
        this.errorMessage = "Something went wrong while fetching attempts from server... Please, try later!";
      }
    })
  ...
  updateRouteWithParams(requestModel: ISolutionsRequest){
    ...
    this.router.navigate([`/tasks/${this.taskId}/attempts`], navigationExtras).then(success => {
      if (success) {
  ...
  getPaginationBackRange(): number[] {
    let lowest = Math.max(1, this.attemptsViewModel.currentPage - 4);
    let highest = this.attemptsViewModel.currentPage;
    return [...this.rangeService.range(lowest, highest)]
  }

  getPaginationFrontRange(): number[] {
    let lowest = this.attemptsViewModel.currentPage + 1;
    let highest = Math.min(this.attemptsViewModel.totalPages, this.attemptsViewModel.currentPage + 4) + 1;
    return [...this.rangeService.range(lowest, highest)]
  }
}
